    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4 text-center">
        <div className="relative mb-6">
          <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 shadow-lg shadow-blue-200">
            <Bot className="h-8 w-8 text-white" />
          </div>
          <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 blur-xl opacity-30 -z-10" />
        </div>
        <p className="text-neutral-900 font-semibold text-xl mb-2">Start a conversation</p>
        <p className="text-neutral-500 text-sm max-w-xs mb-6">Generate React components — buttons, forms, cards, and more</p>
        <div className="flex flex-wrap gap-2 justify-center max-w-xs">
          {["Button with icon", "Login form", "Pricing card", "Data table"].map((prompt) => (
            <span key={prompt} className="px-3 py-1.5 text-xs rounded-full bg-neutral-100 text-neutral-600 border border-neutral-200 hover:bg-blue-50 hover:border-blue-200 hover:text-blue-700 transition-colors cursor-default">
              {prompt}
            </span>
          ))}
        </div>
